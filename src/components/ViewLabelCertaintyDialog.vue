  ...
  <div class="bg-primary" style="min-width: 1000px;">
    <div v-if="dialogLoading">
      <q-bar class="bg-accent text-secondary">
        <span>View Label certainties</span>
        <q-space/>
    ...
    <div v-else>
      ...
      <div class="q-pl-lg q-pr-lg q-pt-md q-pb-sm">
        <q-table title="" :data="studyDetails.instanceSet" :columns="columns" row-key="id" :pagination="initialPagination">
          ...
          <template v-slot:body="props">
            ...
            <q-tr v-show="props.expand" :props="props">
              <q-td colspan="100%">
                <InstanceLabelCertaintyTable :instance="props.row" />
              </q-td>
            </q-tr>
          </template>
        </q-table>
...
import InstanceLabelCertaintyTable from "@/components/InstanceLabelCertaintyTable";

export default {
  name: "ViewLabelCertaintyDialog",
  components: {InstanceLabelCertaintyTable},
  props: {
    'viewId': String,
  },
  data() {
  ...
  methods: {
    async loadLabels() {
      try {
        ...
        this.studyDetails = await GraphQL.getStudyLabelCertainty(this.viewId)
        this.dialogLoading = false
      } catch (e) {
        this.dialogError = true
